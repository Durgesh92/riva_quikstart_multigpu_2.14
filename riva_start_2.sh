...
block_until_server_alive() {
    ...
    for i in {1..30}
    do
    docker exec $1 /bin/grpc_health_probe -addr=:$riva_speech_api_port_new $grpc_tls_args 2> /dev/null
    rc=$?
    if [ $rc -ne 0 ]; then
    ...
    echo "Check Riva logs with: docker logs $riva_daemon_speech_new"
    exit 1
}
...
gpus_to_use_new="device=1"
riva_speech_api_port_new=50052
riva_daemon_speech_new="riva-speech-"$riva_speech_api_port_new
# speech server is required
# check if it's already running first...
if [ $(docker ps -q -f "name=^/$riva_daemon_speech_new$" | wc -l) -eq 0 ]; then
    echo "Starting Riva Speech Services. This may take several minutes depending on the number of models deployed."
    docker rm $riva_daemon_speech_new &> /dev/null
    if [[ $riva_target_gpu_family == "tegra" ]]; then
        docker_run_args="-p 8010:8010 -p 8011:8011 -p 8012:8012 -p 8818:8818 --device /dev/bus/usb --device /dev/snd"
    else
        docker_run_args="-p 8010 -p 8011 -p 8012"
    fi

    echo docker run -d \
        --init \
        --ipc=host \
        --gpus '"'$gpus_to_use_new'"' \
        -p $riva_speech_api_port_new:$riva_speech_api_port_new \
        ...
        --name $riva_daemon_speech_new $docker_run_args \
        $image_speech_api \
        start-riva --riva-uri=0.0.0.0:$riva_speech_api_port_new \
...
block_until_server_alive $riva_daemon_speech_new
...
if [[ $riva_target_gpu_family == "tegra" ]]; then
    if [[ "$2" != "-s" ]]; then
        ...
        docker exec -it $riva_daemon_speech_new /bin/bash
    fi
fi
